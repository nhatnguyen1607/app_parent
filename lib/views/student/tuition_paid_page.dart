...
class TuitionPaidPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalPaid = devMockTuitionPaid.fold<int>(
      0,
      (sum, r) => sum + r.amount,
    );
    final totalRefunds = devMockTuitionRefunds.fold<int>(
      0,
      (sum, r) => sum + r.amount,
    );
                    ...
                    child: Column(
                      children: [
                        DataTable(
                          ...
                          columns: const [
                            DataColumn(label: Text('STT')),
                            DataColumn(label: Text('Số tiền hoàn trả (VNĐ)')),
                            DataColumn(label: Text('Ngày trả')),
                            DataColumn(label: Text('Lý do')),
                          ],
                          rows: [
                            ...List<DataRow>.generate(
                              devMockTuitionRefunds.length,
                              (index) {
                                final r = devMockTuitionRefunds[index];
                                return DataRow(
                                  cells: [
                                    DataCell(Text('${index + 1}')),
                                    DataCell(Text(formatCurrency(r.amount))),
